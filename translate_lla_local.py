...
def main():
    ...
    R_enu_local = read_matrix_from_file('R_ecef_enu_local.txt', 1)
    R_ecef_enu = read_matrix_from_file('R_ecef_enu_local.txt', 2)

    # 假设 P1 是给定的LLA（从其他来源获取）
    P1_LLA = [37.7749, -122.4194, 10.0]  # 例如：某个经纬高
    P1 = geo2ecef(P1_LLA[0], P1_LLA[1], P1_LLA[2])

    # 订阅 "/mavros/gnss_anchor_lla" topic
    rospy.Subscriber('/mavros/gnss_anchor_lla', NavSatFix, gnss_fused_callback)

    # 等待接收消息
    rospy.wait_for_message('/mavros/gnss_anchor_lla', NavSatFix)

    # 计算P3
    P3 = np.linalg.inv(R_ecef_enu).dot(np.linalg.inv(R_enu_local).dot(P1 - P2))
    ...
    # 生成轨迹文件
    generate_trajectory(P5, P4)

def generate_trajectory(P5, P4):
    # 设定四个目标点 (x, y, z)
    points = np.array([
        P5,               # 第一个点是P5
        [P5.[0], P5[1], P5[2]+1.0],  # 中间点，可以根据需要修改
        [P4.[0], P4[1], P4[2]+1.0],  # 中间点，可以根据需要修改
        P4                # 第四个点是P4
    ])
